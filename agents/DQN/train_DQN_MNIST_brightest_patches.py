...
def test_DQN_MNIST_brightest_patches(RLmodel, env, num_epi, TEST_NUM):
	test_R = []
	test_len = []
	for test in range(TEST_NUM):
		...
		# if test<2: env.render(0, os.path.join(RESULT_DIR_TEST, 'XXXtemp_test%i/epi%i'%(test,num_epi)), save=True)
		for t_test in range(env.max_steps):
			action_test, Q_values_test, dist_test, states_test = RLmodel.act(inputs=torch.from_numpy(observation_test).float().resize_(1, 2, 32, 32).to(device), \
			...
			if test<10: 
				...
				env.render(t_test+1, os.path.join(RESULT_DIR_TEST, 'temp_test%i/epi%i'%(test,num_epi)), \
...
if __name__ == '__main__':
	...
	for run in range(RUNS):
		...
		for i_episode in range(NUM_EPISODES):
			...
			if TEST and (i_episode == 0 or (i_episode+1) % TEST_INTERVAL == 0):
				...
				torch.save(net.state_dict(), os.path.join(MODEL_DIR,'model_MNIST_brightest_patches_{NUM_LABELS}labs_{run}runs_{i_episode}epis_{NUM_STEPS}steps_{WINDOW_SIZE}ws.pth'.format(**locals())))
				torch.save(optimizer.state_dict(), os.path.join(MODEL_DIR,'optimizer_MNIST_brightest_patches_{NUM_LABELS}labs_{run}runs_{i_episode}epis_{NUM_STEPS}steps_{WINDOW_SIZE}ws.pth'.format(**locals())))
				...
				if run == 0: # test one run
					...
					test_len, test_R = test_DQN_MNIST_brightest_patches(net, env_test, i_episode, TEST_NUM)
	...
	with open(os.path.join(RESULT_DIR,'run_len_DQN_MNIST_brightest_patches_{NUM_EPISODES}e_{WINDOW_SIZE}w.json'.format(**locals())), 'w') as outfile1:
		json.dump(run_durations, outfile1)

	with open(os.path.join(RESULT_DIR,'run_rewards_DQN_MNIST_brightest_patches_{NUM_EPISODES}e_{WINDOW_SIZE}w.json'.format(**locals())), 'w') as outfile2:
		json.dump(run_total_rewards, outfile2)


	with open(os.path.join(RESULT_DIR,'run_loss_DQN_MNIST_brightest_patches_{NUM_EPISODES}e_{WINDOW_SIZE}w.json'.format(**locals())), 'w') as outfile7:
	...
	plt.xlabel('Episode')
	plt.ylabel('Rewards')
	sns.tsplot(data=[smoothing_average(run_total_rewards[i]) for i in range(len(run_total_rewards))], time=list(range(NUM_EPISODES)), ci=[68, 95], ax=plt.subplot(2, 1, 2), color='red')
