...
class ImgEnv(object):
	def __init__(self, dataset, train, max_steps, channels, window=5, num_labels=10, num_targets=6, reward_shapping='history_brightness'):
		...
		self.reward_shapping = reward_shapping
	...
	def reset(self, NEXT=True):
		if NEXT: # whether switch to next image
			...
			self.max_brightness_so_far = None
		...
		self.targets = self.all_target_patches[:self.num_targets] # top num_targets brightest
	...
	def step(self, action):
		...
		action_brightness = self.all_target_patches_brightness[self.all_target_patches.index(action)]
	
		if self.reward_shapping == 'simple':
			reward = -1. / self.max_steps

		elif self.reward_shapping == 'steps_brightness':
			if self.max_brightness_so_far == None: # first step
				reward = 0.5*(-1. / self.max_steps + action_brightness / self.max_steps)
			else:
				reward = 0.5*(-1. / self.max_steps + max(0, (action_brightness-self.max_brightness_so_far) / self.max_steps))
		else: # 'history_brightness'
			if action in self.action_history:
				# reward = 0.5*(-1. / self.max_steps + (action_brightness-self.max_brightness_so_far) / self.max_steps)
				reward = -1. / self.max_steps
			else:
				if self.max_brightness_so_far == None:
					reward = 0.5*(-1. / self.max_steps + action_brightness / self.max_steps)
				else:
					reward = 0.5*(-1. / self.max_steps + max(0, (action_brightness-self.max_brightness_so_far) / self.max_steps))
		...
		if self.max_brightness_so_far == None or action_brightness > self.max_brightness_so_far:
...
if __name__ == '__main__':
	...
	total_reward = 0
	actions = [0, env.targets[0]-1, env.targets[0]-1, 20, 48, 48]
	for t in range(5):
		action = actions[t]
		next_state, reward, done, info = env.step(action)
		print (action, env.action_history, reward)
		print ('state == next_state', state is next_state)
		# pdb.set_trace()
		# env.render(t, temp_dir = './temp/', done=done, save=False, show_value_image=True, value_image=state[0,:,:])
		state = next_state
